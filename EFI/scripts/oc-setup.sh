...
# install-all / mic / sleep 等会写 ~/Library/LaunchAgents，不能用 root 执行。
# 若用户误用 sudo，自动降权到 SUDO_USER（pmset 子脚本会自行 sudo）。
if [[ "$UID_NUM" -eq 0 ]] && [[ -n "${SUDO_USER:-}" ]] && [[ "${OC_SETUP_AS_USER:-}" != 1 ]]; then
  case "${1:-}" in
    install-all|uninstall-all|status|mic|sleep|lid)
...
Commands:
  install-all              安装 pmset + 耳麦自动切换 + 睡眠 hook（推荐；sudo 亦可，自动降权）
