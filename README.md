...
面向 **HP ZBook Power G7**（**Intel Comet Lake** / 10 代移动平台）的 **OpenCore** Hackintosh EFI。仓库内为当前在用的 **`EFI/oc/`** 配置：含 **`config.plist`**、**ACPI SSDT**（仅 `.aml`）、**Kernel 扩展**及 **`EFI/scripts/`** 合盖关机相关脚本；**`EFI/SysReport/`** 为同机型 ACPI 导出，便于对照修改。

*OpenCore EFI for **HP ZBook Power G7**, Comet Lake, SMBIOS **MacBookPro16,4**, macOS 26 (Darwin 25). iGPU + **WhateverGreen** (`igfxonln=1`); dGPU suppressed (**`-wegnoegpu`**). Wi‑Fi **Intel AX201** via **AirportItlwm_Sequoia** + **IOSkywalkFamily** + **IO80211FamilyLegacy** (OCLP injection for macOS 26); Ethernet **I219-LM** + **IntelMausi**. Trackpad **ELAN** on I²C + **VoodooI2C** / **VoodooI2CHID**, **`-vi2c-force-polling`** (GPIO interrupt unsupported on HP firmware). Sleep: **SSDT-DeepIdle** + AOAC helpers; **`oc-setup.sh`** pmset / 耳麦 / 雷电档位 / 合盖关机等。雷电 **JHL7540** 默认 **lite** 实验档（见下）。*
...
| 无线网 | **Intel Wi‑Fi 6 AX201** → **AirportItlwm_Sequoia** + **IOSkywalkFamily** + **IO80211FamilyLegacy**（OCLP 注入方案，适配 macOS 26 / Darwin 25；`MinKernel` 24.0.0 / `MaxKernel` 25.99.99） |
...
| `EFI/oc/` | OpenCore **`OpenCore.efi`**、**Drivers**（6 个）、**Kexts**、**`config.plist`**、**`ACPI/`**（仅 `.aml` 编译产物）、**Tools**（OpenShell） |
| `EFI/boot/` | 引导相关文件 |
...
- **系统升级**：大版本升级后请核对 **AirportItlwm_Sequoia** 与 **IOSkywalkFamily** / **IO80211FamilyLegacy** 等是否需替换或调整启用范围（以 **`config.plist` → `Kernel` → `Add`** 为准）。**注意**：当前 Wi‑Fi 相关 kext 的 `MaxKernel` 均为 `25.99.99`（即 macOS 26.x）；**升级 macOS 27 前必须先取得支持 Darwin 26 的版本并调整内核范围，否则升级后无 Wi‑Fi**。
...
**无线用哪个驱动？**  
本机 **AX201**：macOS 26 下使用 **AirportItlwm_Sequoia** + **IOSkywalkFamily** + **IO80211FamilyLegacy**（OCLP 注入方案）；具体以当前 **`config.plist`** 里各 kext 的 **`Enabled`** 与内核版本范围为准。
