...
- **★ 核账只认「路径不存在」，严禁代理指标**：APFS 上 `df /` = **只读系统卷**，用户数据在 **`/System/Volumes/Data`**（实测删 4.86 GB 后 `df /` **同一个数**）。**计数类同样失效**（09-20）：`ls *.panic | wc -l` 恒为 **1**，但盘上那个已从 `…-09-17-114214` 换成 **`…-09-18-165628`（X86PlatformShim、开机 T+25 s、与睡眠无关）** ⇒ **必须逐个认文件＋读内容**。
...
- **★★「关 AOAC」= 正式封板**（→ `tier-ladder-why.md` 附录 A–F；台账 `firmware-facts-ledger.md` §8）：**① macOS 声明层**：撤 `\_SB.LPS0` **真跑过** ⇒ 转 S3 ⇒ EC 罢工。**② Win 声明层**：`PlatformAoAcOverride` **确认不存在**（12:0x 直接 `reg query`）；它是 **Windows 自己的键、对 macOS 零帮助** ⇒ **不做 Phase C**。**③ 固件菜单层**：BIOS 里没有；且 12:0x WMI 实测 **`Modern Standby`=`Enable` ＋ `DisplayInUI=0`（隐藏）＋ `IsReadOnly=1`（只读）** ⇒ **"拿到名字就能按名写"作废**（HP 官方 WMI `SetBIOSSetting`/BCU 用同一只读标志）⇒ **固件层没入口 —— 不是"不敢"，是"没门"**。`powercfg /a` 一手原文：S1/S2 带「固件不支持」、**唯独 S3 没有** ⇒ S3=「声明了但被 AOAC 压住」；⚠️ 即便出 S3 也只是**弱阳性**（"声明 ≠ 可用"已在 macOS 半场证过）。
- **★ 固件实锤**（SP169002/01.23.00 首查 ⇒ 09-20 用本机实跑版 01.24.02 复核）：`Modern Standby`＝正式 Setup 项（Enable/Disable、**三语 UI**）、help 自曝互斥；模块 `172 0147` 的 **ASCII 变量名表**含 `DeepS3`+`HpModernStandbyConfigurations`+`PowerControl`（平级）⇒ **互斥＝固件作者设计（AOAC-only 出厂）**；**无 IFR**（三判据全 0）。
- **★ EC = `34.31.00`**（⚠️ **别用 `Win32_BIOS` 的 `52/49`** —— 那是把 BCD `0x34/0x31` 当十进制读的显示产物）⇒ **新规则：HP 出新 BIOS 包先比 EC 号，EC 号不变 ⇒ S3 不会好。**
- **★ 查"某 BIOS 设置的值／藏没藏"的固定路径**（12:0x 实测：**非管理员即可、无需装 HPCMSL、无需重启**）：Win 跑 `root/hp/InstrumentedBIOS` → `HP_BIOSSetting`（**258 项**，带 `DisplayInUI`/`IsReadOnly`）；提示词 `docs/windows-side-workbuddy-prompt.md`，全表存证 `docs/backups/firmware-ledger-2026-09-20/win-side/HPBIOS-all.csv`。
- 现役 **`hibernatemode 0` + `standby 0`**；**配置层已无牌**；**非用户唤醒源 = 0**（17 段普查）；**功耗真值 `6–11 %/h（≈4–7.5 W）`**（"11.2 %/h"=单样本）。工具 `tools/sleep-power-measure.sh`；⚠️ **不许 `pmset schedule wake`**（RTC 写 → POST 005）。
  - **合盖睡眠可行**（`Clamshell.app` 显式 `Software Sleep`）；⛔ 别动 `SSDT-LID-G7`。AC 上 `sleep 0`＋断言 ⇒ **永不自动睡**、空载 ~25 W（常驻叠加，**非电源故障**）；**★ 断言持有者 = WorkBuddy 自己**（`NoIdleSleepAssertion` 自开机起未释放）⇒ **不用就退出它**，否则只能合盖。
