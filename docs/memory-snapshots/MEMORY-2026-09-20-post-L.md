...
- **★ 所有 SSDT 与 `ACPI/Patch` 必须做 `_OSI("Darwin")` 门控**（**OpenCore 打补丁不区分 OS**，会一并作用于 Windows）→ skill **`hackintosh-acpi-os-gating`**、`docs/touchpad-os-gating.md`。（代价：`SSDT-TPD3-PIN` 漏门控改引脚 ⇒ **Windows 触控板失灵**；**修法别写死数字**。09-20 复验：TPD3-PIN／DeepIdle／EC 三张均带 `_OSI` ✅）
...
- **引导器/kext 版本：⚠️ 本机 `nvram …:opencore-version` 不存在**（09-20 实测，`OpenCore.efi` 只有占位符 `REL-XXX-YYYY-MM-DD`）⇒ **该 NVRAM 判据在本机失效**，退回来源记录/OC 日志。跑**开发版** ⇒ 判"是否已修"看 commit/changelog。
...
- **真实 BIOS = `T75 Ver. 01.24.02` / 2026-05-11**（出处 Win hive）⇒ 比 HP 基线 `01.22.00` 新 ⇒ **"固件老旧缺陷"不成立**。（⚠️ **回退策略其实允许**，不刷是因**没收益＋变砖风险**）
...
- 现役 **`hibernatemode 0` + `standby 0`**；**配置层已无牌**；**非用户唤醒源 = 0**（17 段普查）；**功耗真值 `6–11 %/h（≈4–7.5 W）`**。⚠️ **不许 `pmset schedule wake`**（RTC 写 → POST 005）。
  - **合盖睡眠可行**（`Clamshell.app` 显式 `Software Sleep`）；⛔ 别动 `SSDT-LID-G7`。AC 上 `sleep 0`＋断言 ⇒ **永不自动睡**、空载 ~25 W（非故障）；**★ 断言持有者 = WorkBuddy 自己** ⇒ **不用就退出它**。
- **ASPM 已撞墙**（26 条注入已移除），⚠️ 别再当"机会"。
- **★ 两侧功耗差 = 档位差，不是效率差**（→ `round5-win-vs-hack-power.md`、`windows-side-power-prompt.md`）：`powercfg /a` **Win 实测有 S4＋Adaptive Hibernate**（12 h 掉 ≥5% 即自动进 S4≈0 W）⇒ 若也 ~7 %/h 则 **45 min 换档**；macOS `standby 0` ⇒ **只 S0ix 一档**。**Win 功耗从未测**；外部实测 **Win 保 WiFi 连 S0ix 也仅 0.28 W ⇒ 5 W 非 WiFi 锅**。`powercfg /sleepstudy` = macOS 无的"点名凶手"工具。
...
5. **查证三层（顺序固定）**：① 同作者父/邻页 ② 平台汇总仓库问题清单 ③ 用**症状原话**搜论坛；三层无先例才实测，前先写清**预期形态 + 回滚点 + 不可逆风险**。来源分级：`github.com/*`／远景／tonymacx86 = 判据；技术博客／QuickSpecs = 仅方向；**AI 内容农场 = 不可用**。
