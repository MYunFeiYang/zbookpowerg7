...
### 44. 用户要求"压一下 Deep Idle 功耗" —— 实地量真值 + 唯一安全杠杆（§四十四，16:1x）

#### 44.1 本机真实功耗数据（本轮首次实测，非估算）

| 测量项 | 值 | 来源 |
|---|---|---|
| **Awake-idle 墙插功率** | **24.75 W** | `powermetrics -n1 -i3000`（外屏亮 / desktopMode / AC 100%） |
| **Sleep（Deep Idle）功率** | **~5 W（估算，未实测）** | 社区 AOAC 未压降区间 5–10%/h × 70.6 Wh；**本机从未测过真实掉电率** |
| 外接显示器 | **PHL 241B8Q，HDMI，`Main Display=Yes`** | `system_profiler` ⇒ 处于 desktopMode，iGPU 在驱外屏 |
| USB 内核断言 | `HP HD Camera` + `Bluetooth USB Host Controller`（均内置） | `pmset -g assertions`；外接鼠标本次未接 |
| 电源策略 | AC `sleep 0` / `lowpowermode 0`；电池 `lowpowermode 1` | `pmset -g custom` |

**关键认知修正**：之前所有"≈5W≈7%/h"都是**社区区间估算**，本机**从没实测过睡眠掉电率**——所以谈不上"已到地板"，只能说"配置层面已无安全可调项"。

#### 44.2 配置层面为何无杠杆（三层铁证，沿用 §38/§39）

1. **社区 7 条压降清单已用尽**（§三十八）：禁 S3✅ / 关独显➖不适用 / 电源空闲管理✅(=SSDT-DeepIdle 本身) / SSD 品质✅(TLC) / SSD 固件⚠️不建议 / NVMeFix 开 APST✅ / 启用 ASPM(L1)✅(27 条全 `3`=已含 L1)。**唯一被你否的"睡眠关 Wi-Fi/BT"正是最大杠杆**（社区 0.66%/h 署名案例），已排除。
2. **ASPM/NVMe APST 线本机 git 实证已撞墙**（§三十九）：`7b0ab03` 注入后 1.5h 回退 → 收敛到 `ps-max-latency-us`；`3→2` 仅禁 L0s，Deep Idle 链路本就优先 L1，**默认不动**。
3. **EFI 改动全部回滚到稳定态**（§三十四起），睡眠相关变量 = 0。

#### 44.3 唯一安全的真实杠杆：**睡前物理断外设**（零风险，用户动作）

- **拔掉外接显示器（HDMI）**：退出 desktopMode ⇒ iGPU 外屏链路断电 + 显示器自身待机功耗归零（约 0.5–1.5 W 量级，未实测）。
- **拔掉 USB 鼠标/接收器**：消除外接 HID 对 USB 总线的常驻占用。
- 内置 Camera/BT 是平台组成部分，睡眠时会挂起，**不是睡眠功耗来源**（它们的 USB 断言只作用于 awake 态）。
- ⛔ **不动**：Wi-Fi/BT（你已否）、ASPM（撞墙）、SSDT-LID-G7、任何 EFI。

#### 44.4 真正"压"的起点 = 先量真实掉电率（此前从未做）

配置层面已无牌，**下一步必须是测量而不是改**：

```
1. 拔 AC（转电池），记电池 %（如 100%）
2. 合盖睡 4–8 h（Clamshell.app 接管，Deep Idle）
3. 开盖记电池 %  →  掉电%/h × 70.6 Wh = 真实 W
4. 同法再测"拔外屏+拔鼠标"一轮 → 差值 = 外设杠杆的真实收益
```

**无此实数，任何"压"都是盲调**。测完若掉电率已 ≈3%/h 以下（<2.1W），则结构性漏电主导，无可调；若明显更高，再按差值决定要不要动外设。

#### 44.5 结论（可对外收口）

- **配置/EFI 层面 Deep Idle 功耗已无安全可调项**；前两轮"已到地板"表述应改为"**已无安全配置杠杆，真实值待实测**"。
