...
- **触控板**（已解决 `e941e48`）：I2C ELAN073D，`gpioPin=258`(GPP_E2)。OC `ACPI/Patch` 把 DSDT 唯一 `GNUMGPDI`→`TPNMGPDI` + `SSDT-TPD3-PIN.aml`。回滚：加回 `-vi2c-force-polling` + `git revert e941e48`。⚠️ 未 OS 门控 → Win 侧 INT1 也变 258。**［09-18 已兑现并已修］** 该缺口导致 Windows 触控板失灵（用户实测报告）；修法＝`TPNM` 按 `_OSI("Darwin")` 分岔，非 Darwin 回落 `\_SB.GNUM(Arg0)`（原厂值）。详见 `docs/touchpad-os-gating.md`。
